...
twin_analysis <- function(tcr_info, covid_status="any", T_subset="allT", cl_res="cl0.5"){
  ...
  tcr_info = left_join(tcr_info, clust[,c("X", "cl")], by=c("barcode_id"="X"))
  twin_tcrs = get_twin_TCRs(tcr_info[tcr_info$barcode_id %in% cells_included,])
...
process_tcr_info <- function(singleAB = TRUE){
  ...
  #tcr_info = tcr_info[tcr_info$chain_composition == "double_alpha_beta",]
...
get_ccascore_umap <- function(combat_ref, ren_mapped_file, cca_res, CV, side="Y", mp, trunc=NULL, order=FALSE, rev=FALSE){  ##clean up later
  sref = combat_ref
  md = sref$meta_data
  md$UMAP1 = sref$umap$embedding[,1]
  md$UMAP2 = sref$umap$embedding[,2]
